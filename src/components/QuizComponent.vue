		...
		<v-card raised min-height="180px">
			...
			<v-dialog v-model="dialog" persistent max-width="600">
				...
				<div v-if="resultStage">
					...
					<v-card-actions><v-btn @click="playAgain">Done</v-btn></v-card-actions>
				</div>
			</v-card>
		</v-dialog>
...
export default {
	...
	methods: {
		...
		styleSelectedAnswer(index) {
			...
			if (this.selectedIndex != null && index !== this.selectedIndex && this.answers[index] === this.allQandA[this.i].correct_answer) {
		...
		getShuffleAndClean() {
			...
			let dirtyAnswers = [...this.allQandA[this.i].incorrect_answers, this.allQandA[this.i].correct_answer]
